# <code>fragmentomics <b>unlock</b></code>
...
This part of the documentation describes options and concepts for <code>fragmentomics <b>unlock</b></code> sub command in more detail. With minimal configuration, the **`unlock`** sub command enables you to unlock a pipeline output directory. 
...
Unlocking fragmentomics pipeline output directory is fast and easy! In its most basic form, <code>fragmentomics <b>unlock</b></code> only has *one required input*.

## 2. Synopsis
```text
$ ./fragmentomics unlock [-h] --output OUTPUT
```
...
> ***Example:*** `--output /data/$USER/fragmentomics_out`
...
fragmentomics unlock --output /data/$USER/output
```