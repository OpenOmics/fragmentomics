...
# Sentinel value for options.input_type. When set, the concrete input type is
# auto-detected from the user-provided --input files instead of being declared
# up-front. Detection is restricted to the candidates below and enforces a
# single input type per run (no mixing FastQ and BAM).
AUTODETECT_INPUT_TYPE = "auto"
AUTODETECT_CANDIDATES = ("illumina_fastq", "bam")

# Broad FastQ extensions used only for auto-detection of Illumina FastQ input.
# Deliberately more permissive than illumina_fastq's canonical .R1/.R2.fastq.gz
# extensions because sym_safe()/rename() normalize the many accepted FastQ
# naming conventions (_1/_2, _R1_001, .R1.<lane>., ...) to .R1/.R2.fastq.gz.
FASTQ_DETECT_EXTS = (".fastq.gz", ".fq.gz", ".fastq", ".fq")
...
def build_config(sub_args, pl_home):
    ...
    # If the input type is set to auto-detect, resolve it to a single concrete
    # type (illumina_fastq or bam) from the actual --input files. This enforces
    # that a run contains only one input type (no mixing FastQ and BAM).
    if input_type == AUTODETECT_INPUT_TYPE:
        input_type = detect_input_type(sub_args.input)
...
def read_global_options(repo_path):
    ...
    @return (input_type <str>, layout <str>):
        Validated input_type and layout (falls back to auto-detect defaults)
    """
    config_file = os.path.join(repo_path, "config", "config.json")
    # Auto-detect defaults: on a degraded/missing config we prefer auto-detection
    # (fastq or bam) rather than silently assuming a single hard-coded type.
    input_type, layout = AUTODETECT_INPUT_TYPE, "auto"
    try:
...
def validate_input_type(input_type):
    ...
    @param input_type <str>:
        Declared input type (e.g. illumina_fastq, bam, vcf, or auto)
    @return input_type <str>
    """
    # "auto" is a sentinel meaning "detect the type from the --input files"
    # (restricted to AUTODETECT_CANDIDATES). It is resolved to a concrete
    # type in build_config() via detect_input_type() before it is ever used.
    if input_type == AUTODETECT_INPUT_TYPE:
        return input_type
    if input_type not in SUPPORTED_INPUT_FILETYPES:
        raise ValueError(_fatal_message(
            ...
            f"Supported types: {AUTODETECT_INPUT_TYPE}, "
            f"{', '.join(sorted(SUPPORTED_INPUT_FILETYPES))}"
        ))
    return input_type
...
def _looks_like(filename, input_type):
    """Best-effort test of whether a raw (pre-rename) input file belongs to a
    candidate input_type. Used only for auto-detection; authoritative
    validation still happens in validate_inputs() on the renamed symlinks.
    @param filename <str>:
        Raw user-provided input file name or path
    @param input_type <str>:
        Candidate input type to test against
    @return <bool>
    """
    if input_type == "illumina_fastq":
        # More permissive than matches_type() so any accepted FastQ naming
        # convention is detected; rename() normalizes it later. Note that
        # str.endswith() accepts a tuple of suffixes.
        return os.path.basename(filename).lower().endswith(FASTQ_DETECT_EXTS)
    return matches_type(filename, input_type)


def detect_input_type(input_files, candidates=AUTODETECT_CANDIDATES):
    """Auto-detects a single input_type from the user-provided --input files.
    Only the candidate types are considered, and every file must resolve to the
    same type. A mix of types, or any file matching none of the candidates, is
    a fatal error.
    @param input_files list[<str>]:
        Raw user-provided --input files
    @param candidates tuple[<str>]:
        Input types auto-detection is allowed to choose between
    @return <str>:
        The single detected input_type
    """
    detected = {}      # input_type -> [files]
    unrecognized = []  # files matching none of the candidates
    for file in input_files:
        match = next((c for c in candidates if _looks_like(file, c)), None)
        if match is None:
            unrecognized.append(file)
        else:
            detected.setdefault(match, []).append(file)

    # Any file we could not classify is fatal; we cannot safely guess.
    if unrecognized:
        supported = "\n".join(
            f"\t  {c}: {', '.join(extensions_for(c))}" for c in candidates
        )
        raise TypeError(_fatal_message(
            "Could not auto-detect the input type for the following file(s):\n\t"
            + "\n\t".join(unrecognized) + "\n"
            f"Auto-detection supports only these types/extensions:\n{supported}\n"
            "Please provide inputs of a single supported type, or set an "
            "explicit options.input_type in config/config.json."
        ))

    # No classifiable inputs at all (defensive: --input requires >=1 file).
    if not detected:
        raise TypeError(_fatal_message(
            "No inputs were provided to auto-detect an input type from."
        ))

    # A mix of input types is not allowed.
    if len(detected) > 1:
        breakdown = "\n".join(
            "\t{0}:\n\t\t{1}".format(itype, "\n\t\t".join(files))
            for itype, files in detected.items()
        )
        raise TypeError(_fatal_message(
            "Detected a mix of input types, which is not supported. The --input "
            "option accepts either one or more Illumina FastQ files OR one or "
            "more BAM files, but not both:\n" + breakdown + "\n"
            "Please run FastQ and BAM inputs separately (in two different "
            "--output directories)."
        ))
...
def _verify_mates(ifiles):
    ...
    if samples_missing_a_mate:
        raise NameError(_fatal_message(
            "Pipline requires paired-end data but a mate (R1 or R2) is missing for the "
